...
> {
  ...
  public onFriendRequestMessageReceive: (
    message: TMessageFriendRequest,
    state: TState
  ) => void = () => {}
  public onNotficationMessageReceive: (
    message: TMessageNotification,
    state: TState
  ) => void = () => {}
  public onFriendStatusChangeMessageReceive: (
    message: TMessageStatusChange,
    state: TState
  ) => void = () => {}
  public onErrorMessageReceive: (
    message: TMessageError,
    state: TState
  ) => void = () => {}
  public onInitialOnlineFriendsReceive: (
    message: TMessageInitialOnlineUsers,
    state: TState
  ) => void = () => {}
  public onDirectMessageReceive: (
    message: TMessageDirect,
    state: TState
  ) => void = () => {}

  public setMessageReceive(
    handler: (message: TMessage, state: TState) => void
  ) {
    this.onMessageReceive = handler
    return this
  }

  public setFriendRequestMessageReceive(
    handler: (message: TMessageFriendRequest, state: TState) => void
  ) {
    this.onFriendRequestMessageReceive = handler
    return this
  }

  public setNotficationMessageReceive(
    handler: (message: TMessageNotification, state: TState) => void
  ) {
    this.onNotficationMessageReceive = handler
    return this
  }

  public setFriendStatusChangeMessageReceive(
    handler: (message: TMessageStatusChange, state: TState) => void
  ) {
    this.onFriendStatusChangeMessageReceive = handler
    return this
  }

  public setErrorMessageReceive(
    handler: (message: TMessageError, state: TState) => void
  ) {
    this.onErrorMessageReceive = handler
    return this
  }

  public setInitialOnlineFriendsReceive(
    handler: (message: TMessageInitialOnlineUsers, state: TState) => void
  ) {
    this.onInitialOnlineFriendsReceive = handler
    return this
  }

  public setDirectMessageReceive(
    handler: (message: TMessageDirect, state: TState) => void
  ) {
  ...
  public dispatch(message: TMessage) {
    ...
    switch (message.TYPE) {
      ...
      case 'SOCKET_MESSAGE_FRIEND_REQUEST':
        this.onFriendRequestMessageReceive(
          message as TMessageFriendRequest,
          this.state
        )
        break

      case 'SOCKET_MESSAGE_NOTIFICATION':
        this.onNotficationMessageReceive(
          message as TMessageNotification,
          this.state
        )
        break

      case 'SOCKET_MESSAGE_ONLINE_USERS':
        this.onInitialOnlineFriendsReceive(
          message as TMessageInitialOnlineUsers,
          this.state
        )
        break

      case 'SOCKET_MESSAGE_STATUS_CHANGE':
        this.onFriendStatusChangeMessageReceive(
          message as TMessageStatusChange,
          this.state
        )
        break
...
export class MessageEventPublisher {
  ...
  public subscribe<TState extends Record<string, unknown>>(
    subscriber: MessageEventSubscriber<TState>
  ) {
    if (this.subscribers.find((s) => s.id === subscriber.id)) {
      console.warn(
        'You cannot subscribe more than once. Check React render to find unnecessary rerenders',
        subscriber.id
      )
    ...
    this.subscribers.push(
      subscriber as MessageEventSubscriber<Record<string, unknown>>
    )
  }
...
export const WebSocketContextProvider: FC<PropsWithChildren> = ({
  children,
}) => {
  ...
  function establishConnection() {
    ...
    const socket = new WebSocket(
      buildApiUrl(
        `/ws?token=${AuthService.Instance.token}`,
        USE_SSL ? 'wss://' : 'ws://'
      )
    )
    // Connection opened
