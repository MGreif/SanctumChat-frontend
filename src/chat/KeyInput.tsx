...
const tooltipText =
  'Checking this checkbox will save your private key in your browsers local-storage. It may be easier and more comfortable, but having your private key saved might result in leaking your private key if someone manages to access your browser/local-storage.'

export const KeyInput: FC<TKeyInputProps> = ({
  onChange,
  privateKey,
  publicKey,
}) => {
  ...
  return (
    <div className="relative">
      <label htmlFor="key-input">
        <div
          className={`p-4 ${privateKey ? 'bg-indigo-500' : 'bg-red-500'} ${privateKey ? 'hover:bg-indigo-700' : 'hover:bg-red-700'} text-white text-xl rounded-lg hover:bg-indigo-700 cursor-pointer mb-4 text-center`}
          role="button"
        >
          Select RSA private key (.pem)
        </div>
      </label>
      <input
        accept=".pem"
      ...
      <div className="flex gap-2">
        ...
        <Button
          ...
          className="bg-red-500 hover:bg-red-700 w-full flex-1"
        >
          Remove current key
        </Button>
      </div>
      {error && (
        <div className="p-4 border mt-3 border-red-600 bg-red-300 text-center text-xl">
          {error}
        </div>
      )}
