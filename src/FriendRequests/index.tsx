...
import { EHTTPMethod, fetchRequest, useFetchEndpoint } from '../api/fetch.ts'
...
export const FriendRequests: FC = () => {
  ...
  const { data: friendRequests, refetch } = useFetchEndpoint<
    void,
    TApiResponse<TFriendRequest[]>
  >(
    {
  ...
  const handleSendClick: FormEventHandler<HTMLFormElement> = (e) => {
    ...
    if (!value) return
    fetchRequest<{ recipient: string }, { message: string; data: null }>(
      buildApiUrl('/friend-requests'),
      {
      ...
      <div className="mx-auto w-3/4 min-h-0 flex flex-col">
        <div className="flex flex-col md:flex-row gap-8 mb-8">
              ...
              <form onSubmit={handleSendClick}>
                <div className="flex justify-between items-end gap-4">
                  <TextInput
                    ...
                    required
                    ref={inputRef}
                    placeholder={'MaxMuster2000'}
                  />
                  <Button className="bg-indigo-500 w-20" type="submit">
                    Send
                  </Button>
                </div>
              </form>
          ...
          <div className="flex-1 border border-slate-300 p-4 rounded-xl shadow-md h-fit">
            ...
            <p>
              By adding a friend, you are sharing your username and public-key
              with him. Once the recipient approves your request, you can
              exchange messages.
            </p>
            <p>
              If he denies your request, you <b>can not</b> send him a request
              again.
            </p>
          </div>
        </div>
        ...
        <div className="rounded-md min-h-32 overflow-y-auto">
          ...
          {friendRequests?.data?.map((u) => (
            <div className="w-full flex justify-between border border-indigo-300 mb-2 rounded-xl p-3 shadow-sm shadow-indigo-200 items-center">
              <span className={classes.name}>{u.sender_id}</span>
              <span className={classes.buttons}>
                <Button
                  className="bg-red-500 hover:bg-red-700"
                  onClick={() => handleDenyClick(u.id)}
                >
                  Deny
                </Button>
                <Button
                  className="bg-green-500  hover:bg-green-700"
                  onClick={() => handleAcceptClick(u.id)}
                >
                  Accept
                </Button>
              </span>
