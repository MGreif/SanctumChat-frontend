...
export const KeyInput: FC<TKeyInputProps> = ({ onChange }) => {
    ...
    useEffect(() => {
        ...
        onChange(checked ? privateKey : null)
    }, [checked])

    return <>
        <FileInput disabled={checked} mb={"0.5em"} accept={".pem"} label={"Select RSA private key (.pem)"} onChange={file => file && file.text().then(text => text && setPrivateKey(text))} />
        <Checkbox disabled={!privateKey} label={<span>Save in Browser <Tooltip multiline withinPortal style={{ width: "300px", whiteSpace: "pre-wrap", lineBreak: "normal" }} label={tooltipText}><span>(not recommended)</span></Tooltip></span>} checked={checked} onChange={(e) => setChecked(e.target.checked)} />
    </>
