...
export type TUseFriendStatusDebounceState = Record<
  string,
  TFriendStatusUpdateEvent
>
...
export const useFriendStatusDebounce = () => {
  ...
  const dispatch = (
    state: TUseFriendStatusDebounceState,
    username: string,
    event: EEvent,
    callback: () => void,
    timeoutDuration = 200
  ) => {
    const newEvent: TFriendStatusUpdateEvent = {
      event,
    ...
    if (!savedEvent) {
      ...
      if (
        timeDelta < timeoutDuration &&
        newEvent.event === EEvent.ONLINE &&
        savedEvent.event === EEvent.OFFLINE
      ) {
