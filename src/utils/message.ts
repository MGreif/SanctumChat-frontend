...
export const verifyMessagesSignature = (
  ...
  try {
    ...
    const recipientRSAPublic = new Cipher({
      publicKey: fromBase64(recipient_public_key),
    })
    return messages.map((m) => {
      const self_send = m.sender === AuthService.Instance.decodedToken?.sub

      if (self_send) {
        m.message_verified = selfRSAPublic.verifySignature(
          m.message_self_encrypted,
          m.message_self_encrypted_signature
        )
      } else {
        m.message_verified = recipientRSAPublic.verifySignature(
          m.message,
          m.message_signature
        )
      }
...
export const decryptMessages = (
  ...
  try {
    ...
    return messages.map((m) => {
      ...
      if (self_send) {
        m.message_decrypted = selfRSA.decryptWithPrivate(
          m.message_self_encrypted
        )
      } else {
...
export const tryVerifyAndDecryptMessages = (
  senderCipher: Cipher,
